...
#@ String  (label="Objective", choices={"2X", "4X", "10X", "20X"}) objective_id
#@ String  (label="Light-source", choices={"BF", "Fluo1", "Fluo2", "Fluo3", "Fluo4", "Fluo5", "Fluo6"}) lightsource_id
#@ Integer (label="Detection filter", min=1, max=4, step=1) detection_filter

#@ Boolean (label="Run software autofocus", value=True) run_af
#@ String (visibility=MESSAGE, required=false, value="Autofocus settings") doc2
#@ Integer (label="Power (%)", min=0, max=100, value=50) power_af
#@ Integer (label="Exposure (ms)", min=0, value=50) exposure_af
#@ Integer (label="Number of Z-slices", min=0, value=10) nslices_af
#@ Double  (label="Z-step size (µm)", min=0, value=10, style="format:#.#") zstep_af

#@ String (visibility=MESSAGE, required=false, value="Acquisition settings") doc3
...
#@ File (label="Save images in", style="directory") directory
...
class ClickListener(MouseListener):
	...
	def mouseClicked(self, mouseEvent):
		...
		self.im.setMode("script")
		
		objective = dico_objectives[objective_id]
		lightSource = dico_lightsources[lightsource_id]
		
		if run_af:
			
			# Overwrite z_um with zFocus
			# Run AF using Z of the image as center
			z_um = self.im.runSoftwareAutoFocus(objective,	
												lightSource, 
												detection_filter, 
												power_af, 
												exposure_af, 
