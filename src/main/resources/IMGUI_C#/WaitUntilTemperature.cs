 ...
 * Template to wait for the temperature to be in a given interval (matching an exact temperature is a bit time consuming since the temperature has 0.1 precision).
 * Variables at the beginning of the script can be edited (temperature, waiting time...) 
 * 
 * The script starts temperature regulation at the selected temperature, then wait for an initial time, then check if temeprature is in range.
 * If not, wait another duration, defined by the user (pauseTempCheck) before checking again the temperature.
 * 
...
// Define target temperature and allowed deviation, edit as needed
...
while ( sampleTemperature < lowerBoundTemp || upperBoundTemp < sampleTemperature ) // if below lower bound OR above higher bound
{
	...
	Wait(pauseTempCheck.TotalMilliseconds);
...
Wait(waitPostTemp.TotalMilliseconds);
